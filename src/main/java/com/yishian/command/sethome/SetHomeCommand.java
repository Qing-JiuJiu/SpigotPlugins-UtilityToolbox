...
public class SetHomeCommand implements CommandExecutor {
    ...
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        //判断指令是否携带过多参数
        if (args.length > 1) {
            sender.sendMessage(ChatColor.translateAlternateColorCodes('&', PluginMessageConfigEnum.MESSAGE_PREFIX.getMsg() + SetHomeConfigEnum.SETHOME_COMMAND_ERROR.getMsg()));
            return true;
        }
        ...
        if (!allowHomeWorldList.contains(worldName) && !allowHomeWorldList.contains(CommonEnum.ALL.getCommand())) {
            player.sendMessage(ChatColor.translateAlternateColorCodes('&', PluginMessageConfigEnum.MESSAGE_PREFIX.getMsg() + SetHomeConfigEnum.SETHOME_WORLD_ERROR.getMsg()).replaceAll("%world%", worldName));
            return true;
        }
        ...
        //发送设置家成功消息
        player.sendMessage(ChatColor.translateAlternateColorCodes('&', PluginMessageConfigEnum.MESSAGE_PREFIX.getMsg() + SetHomeConfigEnum.SETHOME_APPLY.getMsg()).replaceAll("%name%", homeName));
        return true;
    }
}
