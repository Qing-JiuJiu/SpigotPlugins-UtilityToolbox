    ...
    public void playerOnLeave(PlayerQuitEvent playerQuitEvent) {
        ...
        Bukkit.getScheduler().runTaskAsynchronously(Main.getProvidingPlugin(Main.class), () -> {
            ...
            if (transfeRecordPlayer != null) {
                ...
                transfeRecordPlayer.sendMessage(ChatColor.translateAlternateColorCodes('&', CommonEnum.MESSAGE_PREFIX.getCommand() + TpaCommand.tpaMessage.getString("tpa-others-leave-server").replaceAll("%others-player%", player.getName())));
            }
        });
    }
}
