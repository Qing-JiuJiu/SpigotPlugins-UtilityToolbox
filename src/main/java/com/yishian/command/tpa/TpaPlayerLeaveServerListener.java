    ...
    public void playerOnLeave(PlayerQuitEvent playerQuitEvent) {
        ...
        Bukkit.getScheduler().runTaskAsynchronously(Main.getProvidingPlugin(Main.class), () -> {
            ...
            if (transfeRecordPlayer != null) {
                ...
                transfeRecordPlayer.sendMessage(ChatColor.translateAlternateColorCodes('&', CommonMessageEnum.MESSAGE_PREFIX.getMsg() + TpaConfigEnum.TPA_OTHERS_LEAVE_SERVER.getMsg()).replaceAll("%others-player%", player.getName()));
            }
        });
    }
}
