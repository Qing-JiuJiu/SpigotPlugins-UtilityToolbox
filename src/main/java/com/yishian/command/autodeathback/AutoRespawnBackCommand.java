...
public class AutoRespawnBackCommand implements CommandExecutor {
     ...
     * 允许自动死亡返回列表
     */
    public static ArrayList<UUID> autoDeathBackList = new ArrayList<>();
    ...
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        ...
        ConfigurationSection autoDeathBackMessage = configurationSection.getConfigurationSection(autoRespawnBackCommand).getConfigurationSection(CommonEnum.MESSAGE.getCommand());

        //判断执行指令的是用户还是控制台
        if (!(sender instanceof Player)) {
            sender.sendMessage(ChatColor.translateAlternateColorCodes('&', messagePrefix + autoDeathBackMessage.getString("autorespawnback-console-error")));
            return true;
        }
