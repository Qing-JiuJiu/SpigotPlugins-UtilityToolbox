        ...
        if (sender.hasPermission(CommandEnum.RELOAD_CONFIG_PERMISSION.getCommand())) {
            if (CommandEnum.PLUGHIN_NAME.getCommand().equalsIgnoreCase(label)) {
                if (args.length == 1 && CommandEnum.RELOAD_CONFIG_COMMAND.getCommand().equalsIgnoreCase(args[0])) {
                    ...
                    sender.sendMessage(messagePrefix + pluginMessage.getString("reload-message"));
                } else {
                    sender.sendMessage(messagePrefix + pluginMessage.getString("command-args-error"));
                }
            }
        } else {
            sender.sendMessage(messagePrefix + pluginMessage.getString("command-no-permission"));
        }
        return true;
    }
